...
func newHTTPServerForTest(t *testing.T) (*Server, *db.Client) {
	...
	projectService := projectsvc.New(cfg, dbClient, ps)
	issueService := issuesvc.New(dbClient)
	issueRunService := issuerunsvc.New(dbClient)
	opsService := opssvc.New(dbClient)
	return New(cfg, log, dbClient, userService, projectService, issueService, issueRunService, opsService), dbClient
}
...
func TestServerEndpointsIntegration(t *testing.T) {
	...
	gitlabServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet && r.URL.EscapedPath() == "/api/v4/projects/group%2Fp-http" {
			if got := r.Header.Get("PRIVATE-TOKEN"); got != "token-http" {
				t.Fatalf("gitlab validator PRIVATE-TOKEN mismatch: %q", got)
			}
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"id":701,"path_with_namespace":"group/p-http"}`))
			return
		}
	...
	createProjectBody := fmt.Sprintf(`{
		"project_key":"p-http",
		"project_slug":"group/p-http",
		"name":"Project HTTP",
		"provider":"gitlab",
		"provider_url":"%s/api/v4",
		"repo_url":"%s/group/p-http",
		"default_branch":"main",
		"project_token":"token-http",
		"sandbox_plan_review":false,
		"poll_interval_sec":30,
		"enabled":true
	}`, gitlabServer.URL, gitlabServer.URL)
