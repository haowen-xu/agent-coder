...
const missingWebUIFallbackHTML = "<!doctype html><html><head><meta charset=\"utf-8\"><title>WebUI Assets Missing</title></head><body><h1>WebUI assets missing</h1><p>Run <code>cd webui && pnpm build</code> before debugging or packaging.</p></body></html>"
...
func (s *Server) handleNoRoute(_ context.Context, c *app.RequestContext) {
	...
	if strings.HasPrefix(p, "/api/") {
		httputil.WriteError(c, http.StatusNotFound, "route not found")
		return
	}
	s.serveSPA(c, p)
}
...
func (s *Server) serveSPA(c *app.RequestContext, rawPath string) {
	...
	if err != nil {
		...
		if err != nil {
			...
			c.Write([]byte(missingWebUIFallbackHTML))
			return
		}
		c.SetStatusCode(http.StatusOK)
		c.Header("Content-Type", "text/html; charset=utf-8")
