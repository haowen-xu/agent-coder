...
// TestStaticFallbackWhenEmbeddedAssetsMissing 用于单元测试。
func TestStaticFallbackWhenEmbeddedAssetsMissing(t *testing.T) {
	t.Parallel()

	s, _ := newHTTPServerForTest(t)
	original := webuiFS
	webuiFS = embed.FS{}
	t.Cleanup(func() { webuiFS = original })

	rec := httpReq(s, "GET", "/", "", "")
	if rec.Code != 200 {
		t.Fatalf("expected 200 for fallback index, got %d body=%s", rec.Code, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), "pnpm build") {
		t.Fatalf("expected fallback html to contain pnpm build hint, got body=%s", rec.Body.String())
	}
}
